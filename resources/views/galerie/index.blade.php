            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-md sm:rounded-lg">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-100 dark:bg-gray-700">
                        <tr>
                            <th class="px-6 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200">Nom évènement</th>
                            <th class="px-6 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200">Description</th>
                            <th class="px-6 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200">Photo de couverture</th>
                            <th class="px-6 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200">Photos</th>
                        ...
                        @forelse ($galeries as $galerie)
                            <tr class="border-b border-gray-200 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 text-gray-900 dark:text-white">
                                    <div class="flex items-center space-x-1">
                                        ...
                                        @forelse($previewPhotos as $photo)
                                            <img src="{{ $photo->getUrl() }}" alt="photo" class="w-10 h-10 object-cover rounded shadow">
                                        @empty
                                            <span class="text-sm text-gray-500 dark:text-gray-400">Aucune</span>
                                        @endforelse

                                        @if($photos->count() > 3)
                                            <span class="text-sm text-gray-600 dark:text-gray-300">+{{ $photos->count() - 3 }} autre{{ $photos->count() - 3 > 1 ? 's' : '' }}</span>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 space-x-4">
                                    <a href="{{ route('galerie.show', $galerie) }}" class="text-yellow-500 hover:underline"><i class="bi bi-eye-fill">Voir</i></a>
                                    <a href="{{ route('galerie.edit', $galerie) }}" class="text-indigo-500 hover:underline"><i class="bi bi-pencil-fill">Modifier</i></a>
                                    <form action="{{ route('galerie.destroy', $galerie) }}" method="POST" class="inline"
                                          onsubmit="return confirm('Supprimer cette galerie ?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-600 hover:underline"><i class="bi bi-trash-fill">Supprimer</i></button>
                                    </form>
                                </td>
                            </tr>
                        @empty
